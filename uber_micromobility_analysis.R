...
MidtownDD = MidtownDDD[MidtownDDD$origin.area %in% c("m","c"),]
...
figure_1b <- ggplot()+ 
  ...
  geom_sf(data = Atlanta.sub, aes( geometry=geometry, fill=Counterfactual),lwd = 0.2, color = "black")+
  geom_point(data = marta, aes(x = long, y = lat), shape = 16, fill = "black", size=8)+
...
MARTA_est <- lm_robust(Travel.Time.per.Mile..Minutes.Mile. ~ treatment +
                         post + post*treatment + as.factor(month) +
                         Vehicles + TransRoutes + WalkScore + NumBikeshare +
                         school.population  + event + as.factor(dayofweek) +
                         Precip + festival, data = MARTA, cluster = as.factor(origin.tract), se_type = "stata")
summary(MARTA_est)
...
est_MARTA <- felm(Travel.Time.per.Mile..Minutes.Mile. ~ treatment + post + post*treatment + month + Vehicles + TransRoutes + WalkScore + NumBikeshare + school.population + event + as.factor(dayofweek) + Precip + festival | 0 | 0 | origin.tract, data = MARTA, cmethod = "cgm2")
summary(est_MARTA)
...
MidtownDD_est = lm_robust(traveltime.mile ~ treated+ post + treated*post + as.factor(dayofweek) + as.factor(month)+ numvehicles + transit.routes + walk.score + bike.hubs+ totalschool + Precip + festival,
...
est_MidtownDD <- felm(traveltime.mile ~ treated+ post + treated*post +
                        numvehicles + transit.routes + walk.score +
                        bike.hubs+ totalschool + Precip + as.factor(month) + as.factor(dayofweek) + festival | 0 | 0 |origin.tract,
...
MidtownDDD_est<-lm_robust(traveltime.mile ~ treated+ post + scooter + treated*post + treated*scooter+ post*scooter +
                            ...
                            as.factor(dayofweek) + Precip + festival, data=MidtownDDD, clusters=origin.tract, se_type = "stata")
...
est_MidtownDDD <- felm(traveltime.mile ~ treated+ post + scooter + treated*post + treated*scooter+ post*scooter + treated*post*scooter + numvehicles + transit.routes + walk.score + bike.hubs +
                         Precip + totalschool+ as.factor(dayofweek) + as.factor(month) + festival | 0 | 0 | origin.tract,
...
for(i in 0:44) {
  ...
  if (start > as.Date("2019-09-13")) {
    reg <- lm_robust(traveltime.mile ~ treated+ post + scooter +
                       treated*post + treated*scooter+ post*scooter +
                       treated*post*scooter + as.factor(dayofweek) +
                       numvehicles + transit.routes + walk.score +
                       bike.hubs + totalschool + Precip + festival,
                     data=subset, se_type = "stata")
  } else {
    reg <- lm_robust(traveltime.mile ~ treated+ post + scooter +
                       treated*post + treated*scooter+ post*scooter +
                       treated*post*scooter + as.factor(dayofweek) +
                       numvehicles + transit.routes + walk.score + bike.hubs +
                       totalschool + Precip, data=subset, se_type = "stata")
  }
  start <- start + 1
  ...
  observations <- c(observations, as.numeric(reg$nobs))
}
...
table_s1<- matrix(nrow = 5,ncol = 13, dimnames = list(c("Effect Size","SE", "p-value", "F-stat", "n"),
                                                      c("1 day","2 days", "3 days", "4 days", "5 days", "10 days", "15 days", "20 days", "25 days", "30 days", "35 days", "40 days", "45 days")))

table_s1[1,] <- c(round(coeffs[1],3), round(coeffs[2],3), round(coeffs[3],3), round(coeffs[4],3), round(coeffs[5],3), round(coeffs[10],3), round(coeffs[15],3), round(coeffs[20],3), round(coeffs[25],3), round(coeffs[30],3), round(coeffs[35],3), round(coeffs[40],3), round(coeffs[45],3))
table_s1[2,] <- c(round(se[1],3), round(se[2],3), round(se[3],3), round(se[4],3), round(se[5],3), round(se[10],3), round(se[15],3), round(se[20],3), round(se[25],3), round(se[30],3), round(se[35],3), round(se[40],3), round(se[45],3))
table_s1[3,] <- c(round(p[1],3), round(p[2],3), round(p[3],3), round(p[4],3), round(p[5],3), round(p[10],3), round(p[15],3), round(p[20],3), round(p[25],3), round(p[30],3), round(p[35],3), round(p[40],3), round(p[45],3))
table_s1[4,] <- c(f[1], f[2], f[3], f[4], f[5], f[10], f[15], f[20], f[25], f[30], f[35], f[40], f[45])
table_s1[5,] <- c(observations[1], observations[2], observations[3], observations[4], observations[5], observations[10], observations[15], observations[20], observations[25], observations[30], observations[35], observations[40], observations[45])
...
for(i in 0:44) {
  ...
  if (start > as.Date("2019-09-13")) {
    reg_marta <- lm_robust(Travel.Time.per.Mile..Minutes.Mile. ~ treatment +
                             post + post*treatment +  Mon + Wed + Thur + Fri +
                             Sat + Sun + Vehicles + TransRoutes + WalkScore +
                             NumBikeshare + school.population  + event + Precip + festival,
                           data = subset, se_type = "stata")
  } else {
    reg_marta <- lm_robust(Travel.Time.per.Mile..Minutes.Mile. ~ treatment +
                             post + post*treatment +  Mon + Wed + Thur + Fri +
                             Sat + Sun + Vehicles + TransRoutes + WalkScore +
                             NumBikeshare + school.population  + event + Precip,
                           data = subset, se_type = "stata")
  }
  start <- start + 1
  ...
  observations_marta <- c(observations_marta, as.numeric(reg_marta$nobs))
}
...
table_s2<- matrix(nrow = 5,ncol = 13, dimnames = list(c("Effect Size","SE", "p-value", "F-stat", "n"),
                                                      c("1 day","2 days", "3 days", "4 days", "5 days", "10 days", "15 days", "20 days", "25 days", "30 days", "35 days", "40 days", "45 days")))

table_s2[1,] <- c(round(coeffs_marta[1],3), round(coeffs_marta[2],3), round(coeffs_marta[3],3), round(coeffs_marta[4],3), round(coeffs_marta[5],3), round(coeffs_marta[10],3), round(coeffs_marta[15],3), round(coeffs_marta[20],3), round(coeffs_marta[25],3), round(coeffs_marta[30],3), round(coeffs_marta[35],3), round(coeffs_marta[40],3), round(coeffs_marta[45],3))
table_s2[2,] <- c(round(se_marta[1],3), round(se_marta[2],3), round(se_marta[3],3), round(se_marta[4],3), round(se_marta[5],3), round(se_marta[10],3), round(se_marta[15],3), round(se_marta[20],3), round(se_marta[25],3), round(se_marta[30],3), round(se_marta[35],3), round(se_marta[40],3), round(se_marta[45],3))
table_s2[3,] <- c(round(p_marta[1],3), round(p_marta[2],3), round(p_marta[3],3), round(p_marta[4],3), round(p_marta[5],3), round(p_marta[10],3), round(p_marta[15],3), round(p_marta[20],3), round(p_marta[25],3), round(p_marta[30],3), round(p_marta[35],3), round(p_marta[40],3), round(p_marta[45],3))
table_s2[4,] <- c(f_marta[1], f_marta[2], f_marta[3], f_marta[4], f_marta[5], f_marta[10], f_marta[15], f_marta[20], f_marta[25], f_marta[30], f_marta[35], f_marta[40], f_marta[45])
table_s2[5,] <- c(observations_marta[1], observations_marta[2], observations_marta[3], observations_marta[4], observations_marta[5], observations_marta[10], observations_marta[15], observations_marta[20], observations_marta[25], observations_marta[30], observations_marta[35], observations_marta[40], observations_marta[45])

#-------------------------------------------
#TABLE S3
#-------------------------------------------

table_s3<- matrix(nrow = 7,ncol = 5, dimnames = list(c("Midtown","Buckhead", "Cumberland", "Sandy Springs", "Policy zone subway", "No policy zone subway", "Stadium"),
                                                     c("Mean","SD", "Min", "Max", "N")))

table_s3[1,] <- c(mean(subset(MidtownDDD, origin.area == 'm')$traveltime.mile),
...
table_s3[2,] <- c(mean(subset(MidtownDDD, origin.area == 'c')$traveltime.mile),
...
table_s3[3,] <- c(mean(subset(MidtownDDD, origin.area == 'b')$traveltime.mile),
...
table_s3[4,] <- c(mean(subset(MidtownDDD, origin.area == 's')$traveltime.mile),
...
table_s3[5,] <- c(mean(subset(MARTA, treatment == 1)$Travel.Time.per.Mile..Minutes.Mile.),
...
table_s3[6,] <- c(mean(subset(MARTA, treatment == 0)$Travel.Time.per.Mile..Minutes.Mile.),
...
table_s3[7,] <- c(mean(MB$dependent),
                  sd(MB$dependent),
                  min(MB$dependent), 
                  max(MB$dependent),
                  length(MB$dependent))
...
table_s4<- matrix(nrow = 14,ncol = 8, dimnames = list(c("Pop","Med Age", "Med Inc", "Vehicles","Ed. Attainment", "Pct. Nonwhite", "Transit score", "No. routes", "Walk score", "No. bike hubs","Travel Time", "Tot enrollment", "K-12", "College"),
                                                      c("Mean, Midtown","SD, Midtown", "Min, Midtown", "Max, Midtown","Mean, CF","SD, CF", "Min, CF", "Max, CF")))

table_s4[1,] <- c(mean(subset(covs, origin.area == 'm')$Population),
...
table_s4[2,] <- c(mean(subset(covs, origin.area == 'm')$MedianAge),
...
table_s4[3,] <- c(mean(subset(covs, origin.area == 'm')$MedianIncome),
...
table_s4[4,] <- c(mean(subset(covs, origin.area == 'm')$veh.pop),
...
table_s4[5,] <- c(mean(subset(covs, origin.area == 'm')$Bachelors),
...
table_s4[6,] <- c(mean(subset(covs, origin.area == 'm')$minorityShare),
...
table_s4[7,] <- c(mean(subset(covs, origin.area == 'm')$transit.score),
...
table_s4[8,] <- c(mean(subset(covs, origin.area == 'm')$transit.routes),
...
table_s4[9,] <- c(mean(subset(covs, origin.area == 'm')$walk.score),
...
table_s4[10,] <- c(mean(subset(covs, origin.area == 'm')$bike.hubs),
...
table_s4[11,] <- c(mean(subset(covs, origin.area == 'm')$AggTravelTime),
...
table_s4[12,] <- c(mean(subset(covs, origin.area == 'm')$total.school),
...
table_s4[13,] <- c(mean(subset(covs, origin.area == 'm')$k12),
...
table_s4[14,] <- c(mean(subset(covs, origin.area == 'm')$college),
...
S7_DD1 = lm_robust(as.numeric(traveltime.mile) ~ treated+ post +
                     treated*post + as.factor(dayofweek) + as.factor(month)+
                     numvehicles + transit.routes + walk.score + bike.hubs +
                     Precip + festival,
                   data=MidtownDD, se_type = "stata", clusters=MidtownDD$origin.tract)   

S7_DD2 = lm_robust(as.numeric(traveltime.mile) ~ treated+ post +
                     treated*post + as.factor(dayofweek) + as.factor(month)+
                     numvehicles + transit.routes + walk.score + bike.hubs + totalschool +
                     Precip + festival,
                   data=MidtownDD, se_type = "stata", clusters=MidtownDD$origin.tract)
...
S7_DDD1 <-lm_robust(traveltime.mile ~ treated+ post + scooter + treated*post + treated*scooter+ post*scooter +
                      ...
                      Precip + festival + as.factor(dayofweek), data=MidtownDDD, clusters=origin.tract, se_type = "stata")
...
S7_MARTA1 <- lm_robust(Travel.Time.per.Mile..Minutes.Mile. ~ treatment 
                       ...
                       + event + festival + as.factor(dayofweek) + Precip, data = MARTA,
...
table_s6<- matrix(nrow = 8,ncol = 2, dimnames = list(c("Midtown Experiment, No School Controls",
...
table_s6[,1] = c(S7_DD1$coefficients[["treatedTRUE:postTRUE"]],
...
table_s6[,2] = c(round(S7_DDD1$coefficients[["treatedTRUE:postTRUE:scooterTRUE"]],3),
...
S6_DD1 <- felm(as.numeric(traveltime.mile) ~ treated+ post + treated*post +
                        numvehicles + transit.routes + walk.score +
                        bike.hubs+ totalschool + Precip + festival | 0 | 0 |origin.tract,
                      data = MidtownDD, cmethod = "cgm2")
summary(S6_DD1)

S6_DD2 <- felm(as.numeric(traveltime.mile) ~ treated+ post + treated*post +
                 numvehicles + transit.routes + walk.score +
                 bike.hubs+ totalschool + Precip + festival | 0 | 0 |origin.tract + dayofweek,
               data = MidtownDD, cmethod = "cgm2")
summary(S6_DD2)

S6_DD3 <- felm(as.numeric(traveltime.mile) ~ treated+ post + treated*post +
                 numvehicles + transit.routes + walk.score +
                 bike.hubs+ totalschool + Precip + festival | 0 | 0 |origin.tract + weeknumber,
               data = MidtownDD, cmethod = "cgm2")
summary(S6_DD3)
...
S6_DDD1 <- felm(traveltime.mile ~ treated+ post + scooter + treated*post + treated*scooter+ post*scooter + treated*post*scooter + numvehicles + transit.routes + walk.score + bike.hubs +
                         Precip + totalschool + festival | 0 | 0 | origin.tract,
                       data = MidtownDDD, cmethod = "cgm2")
summary(S6_DDD1)
...
S6_DDD2 <- felm(traveltime.mile ~ treated+ post + scooter + treated*post + treated*scooter+ post*scooter + treated*post*scooter + numvehicles + transit.routes + walk.score + bike.hubs +
                  Precip + totalschool + festival | 0 | 0 | origin.tract + dayofweek,
                data = MidtownDDD, cmethod = "cgm2")
summary(S6_DDD2)
...
S6_DDD3 <- felm(traveltime.mile ~ treated+ post + scooter + treated*post + treated*scooter+ post*scooter + treated*post*scooter + numvehicles + transit.routes + walk.score + bike.hubs +
                  Precip + totalschool + festival | 0 | 0 | origin.tract + weeknumber,
                data = MidtownDDD, cmethod = "cgm2")
summary(S6_DDD3)

table_s7<- matrix(nrow = 6,ncol = 2, dimnames = list(c("One-way (origin tract)","SE 1", "Two-way (origin tract and day of week)", "SE 2", "Two-way (origin tract and week number)", "SE 3"),
                                                     c("DD Estimator", "DDD Estimator")))

table_s7[,1] = c(S6_DD1$coefficients["treatedTRUE:postTRUE",],
...
table_s7[,2] = c(S6_DDD1$coefficients["treatedTRUE:postTRUE:scooterTRUE",],
...
DDD2018_2 <- felm(minutes_per_mile ~ treated+ post + scooter + treated*post + treated*scooter+ post*scooter +
                    treated*post*scooter + numvehicles +
                    transit.routes + walk.score + bike.hubs + totalschool + precip + day_of_week 
                  | 0 | 0 | origin_tract + month, data = midtownddd_2018, cmethod = "cgm2")

DDD2018_3 <- felm(minutes_per_mile ~ treated+ post + scooter + treated*post + treated*scooter+ post*scooter +
                    treated*post*scooter + numvehicles + month +
                    transit.routes + walk.score + bike.hubs + totalschool + precip 
                  | 0 | 0 | origin_tract + day_of_week, data = midtownddd_2018, cmethod = "cgm2")
...
MidtownDD_P1 <- felm(as.numeric(traveltime.mile) ~ treated+ post.placebo + treated*post.placebo +
                       numvehicles + festival + transit.routes + walk.score + bike.hubs+ totalschool + Precip
                     | 0 | 0 | origin.tract,
                     data = MidtownDD_preonly, cmethod = "cgm2")
summary(MidtownDD_P1)

MidtownDD_P2 <- felm(as.numeric(traveltime.mile) ~ treated+ post.placebo + treated*post.placebo +
                       numvehicles + festival + transit.routes + walk.score + bike.hubs+ totalschool + Precip
                     | 0 | 0 | origin.tract + month,
                     data = MidtownDD_preonly, cmethod = "cgm2")
summary(MidtownDD_P2)

MidtownDD_P3 <- felm(as.numeric(traveltime.mile) ~ treated+ post.placebo + treated*post.placebo +
                       numvehicles + festival + transit.routes + walk.score + bike.hubs+ totalschool + Precip
                     | 0 | 0 | origin.tract + dayofweek,
                     data = MidtownDD_preonly, cmethod = "cgm2")
...
MARTA_P1 <- felm(Travel.Time.per.Mile..Minutes.Mile. ~ treatment + post.placebo +
                   ...
                   NumBikeshare + festival + school.population + event + Precip
                 | 0 | 0 | origin.tract,
